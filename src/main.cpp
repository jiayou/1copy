...
void print_dup_json(vector<Duplication> dup)
{
	...
	for (auto& d:dup){
		...
		cout << "\t" << "filename: \"" << ws2s(d.filename) << "\"," << endl;
		cout << "\t" << "filesize: " << std::to_string(d.filesize) << "," << endl;
		cout << "\t" << "paths: [" << endl;
		for (auto& p:d.paths) {
...
int main(int argc, const char** argv) 
{
	...
	parser.add_option("-z", "--undo")
										.action("store_true") .dest("undo") .set_default(false)
										.help("cancel removing duplications");
	parser.add_option("-v", "--verbose")
										.action("count") .dest("verbose")
										.help("display debug outputs");
	parser.add_option("-n", "--dry-run") 
										.action("store_true") .dest("dryrun") .set_default(true)
										.help("display, do NOT change anything");
	parser.add_option("-x", "--exec") 
										.action("store_false") .dest("dryrun") .set_default(false)
										.help("execute");
	...
	if (verbose-- > 0) spdlog::set_level(spdlog::level::info);
	if (verbose-- > 0) spdlog::set_level(spdlog::level::debug);
	if (verbose-- > 0) spdlog::set_level(spdlog::level::trace);
